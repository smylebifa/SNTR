      ...
      if ($code_of_competency_apply === '') {
        ...
        if ($code_of_competency_service === '') 
        {
          $sql_select = $wpdb->get_results($wpdb->prepare("
            ...
            RIGHT JOIN ОквэдВЧастиПрименения
            ...
            WHERE КодыОквэд.КодОквэд = %s
            AND ЦентрыКомпетенций.НазваниеКомпетенции IS NOT NULL", $code_of_competency_apply));
        }
        ...
        else 
        {
         $sql_select = $wpdb->get_results($wpdb->prepare("
          ...
          AND ЦентрыКомпетенций.НазваниеКомпетенции IS NOT NULL", [$code_of_competency_apply, $code_of_competency_service]));
       }
    ...
    if ($code_of_competency_apply === '') {
      ...
      if ($code_of_competency_service === '') {
        $sql_select = $wpdb->get_results($wpdb->prepare("
          ...
          WHERE КодыОквэд.КодОквэд = %s
          AND ЦентрыКомпетенций.НазваниеКомпетенции IS NOT NULL", $code_of_competency_dev));
      }
      ...
      // 101
        ...
        $sql_select = $wpdb->get_results($wpdb->prepare("
         ...
         WHERE КодыОквэд.КодОквэд = %s
         AND КодыОквэд.КодОквэд = %s
         AND ЦентрыКомпетенций.НазваниеКомпетенции IS NOT NULL", [$code_of_competency_dev, $code_of_competency_service]));
      }
      ...
      if ($code_of_competency_service === '') 
      {
        $sql_select = $wpdb->get_results($wpdb->prepare("
         ...
         WHERE КодыОквэд.КодОквэд = %s
         AND КодыОквэд.КодОквэд = %s
         AND ЦентрыКомпетенций.НазваниеКомпетенции IS NOT NULL", [$code_of_competency_dev, $code_of_competency_apply]));
      }
      ...
      else 
      {
        $sql_select = $wpdb->get_results($wpdb->prepare("
         ...
         AND КодыОквэд.КодОквэд = %s
         AND КодыОквэд.КодОквэд = %s
         AND ЦентрыКомпетенций.НазваниеКомпетенции IS NOT NULL", [$code_of_competency_dev, $code_of_competency_apply, $code_of_competency_service]));
      }
    }
  }
